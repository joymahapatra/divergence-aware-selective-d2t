...
def parent_precision(prediction,
           reference,
           table,
           lambda_weight=0.5,
           smoothing=0.00001,
...
def parent_srd(reference,
    ...
    for order in range(1, max_order + 1):
        ...
        for ngram, count in ref_ngram_counts.items():
            ...
            prob_ngram_in_ref = min(
                1., float(ref_ngram_counts.get(ngram, 0) / count))
            # numerator += count * (prob_ngram_in_ref)
...
class SRDivergence:
    ...
    def get_groups(self, text, is_verb=True):
        """ne, noun, and verb extractor"""
        text = text.lower()
        doc = self.spacy_model(text)
        bucket = []
        for ent in doc.ents:
            bucket.append(ent.text) # type `ent.label_`
        for token in doc:
            if token.pos_ == "VERB" and is_verb:
                bucket.append(token.text)
            if token.pos_ == "NOUN":
                bucket.append(token.text)
        return bucket
                
    def measure(self, structure: list, reference: str) -> dict[str, float]:
        text_structure = ""
        for i in structure:
            text_structure = text_structure + " ".join(list(i))
        text_structure = text_structure.lower()
        reference = reference.lower()
        set_structure = set(self.tokenizer(text=text_structure))
        set_reference = set(self.tokenizer(text=reference))
        set_group_reference = set(self.get_groups(text=reference))
        unigram_measure = len(set_reference.intersection(set_structure))/(len(set_reference.union(set_structure)) + self.epsilon)
        unigram_group_measure = len(set_group_reference.intersection(set_structure))/(len(set_group_reference) + self.epsilon)
        # doing for parent
        parent_input_table = []
        for i in structure:
            tmp_field = []
            for j in i:
                tmp_field.append(self.tokenizer(text=j))
            tmp_field = tuple(tmp_field)
            parent_input_table.append(tmp_field)
        parent_input_reference = self.tokenizer(text=reference)
        # print(f"parent-table: {parent_input_table}, parent-reference: {parent_input_reference}")
        # print(f"set structure: {set_structure}, set reference: {set_reference}, and set group reference: {set_group_reference}")
        return {'unigram_srd': unigram_measure, 
                'unigram_ne_srd': unigram_group_measure, 
                'parent_srd': parent_srd(reference=parent_input_reference, table=parent_input_table)}
